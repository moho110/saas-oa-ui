    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['xinhu-finance:custappy:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['xinhu-finance:custappy:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['xinhu-finance:custappy:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['xinhu-finance:custappy:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="custappyList" @selection-change="handleSelectionChange">
...
<script>
import { listCustappy, getCustappy, delCustappy, addCustappy, updateCustappy } from "@/api/xinhu-finance/custappy";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listCustappy(this.queryParams).then(response => {
        this.custappyList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCustappy(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addCustappy(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除客户申请使用编号为"' + ids + '"的数据项？').then(function() {
        return delCustappy(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('xinhu-finance/custappy/export', {
        ...this.queryParams
      }, `custappy_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
