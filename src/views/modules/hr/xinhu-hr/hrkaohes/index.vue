    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['xinhu-hr:hrkaohes:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['xinhu-hr:hrkaohes:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['xinhu-hr:hrkaohes:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['xinhu-hr:hrkaohes:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="hrkaohesList" @selection-change="handleSelectionChange">
...
<script>
import { listHrkaohes, getHrkaohes, delHrkaohes, addHrkaohes, updateHrkaohes } from "@/api/xinhu-hr/hrkaohes";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listHrkaohes(this.queryParams).then(response => {
        this.hrkaohesList = response.rows;
    ...
    handleAdd() {
      this.reset();
      this.open = true;
      this.title = "添加考核项目的内容";
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset();
      const id = row.id || this.ids
      getHrkaohes(id).then(response => {
        this.form = response.data;
        this.open = true;
        this.title = "修改考核项目的内容";
      });
    },
    /** 提交按钮 */
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateHrkaohes(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addHrkaohes(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除考核项目的内容编号为"' + ids + '"的数据项？').then(function() {
        return delHrkaohes(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('xinhu-hr/hrkaohes/export', {
        ...this.queryParams
      }, `hrkaohes_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
