<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="要调动人" prop="tranname">
        <el-input
          v-model="queryParams.tranname"
          placeholder="请输入要调动人"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
      ...
      <el-form-item label="原来部门" prop="olddeptname">
        <el-input
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['xinhu-hr:hrtransfer:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['xinhu-hr:hrtransfer:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['xinhu-hr:hrtransfer:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['xinhu-hr:hrtransfer:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="调动人ID" prop="tranuid">
          <el-input v-model="form.tranuid" placeholder="请输入调动人ID" />
        </el-form-item>
        <el-form-item label="要调动人" prop="tranname">
          <el-input v-model="form.tranname" placeholder="请输入要调动人" />
        </el-form-item>
        <el-form-item label="原来部门" prop="olddeptname">
          <el-input v-model="form.olddeptname" placeholder="请输入原来部门" />
        </el-form-item>
        <el-form-item label="原来职位" prop="oldranking">
          <el-input v-model="form.oldranking" placeholder="请输入原来职位" />
        </el-form-item>
        ...
        <el-form-item label="调动后部门" prop="newdeptname">
          <el-input v-model="form.newdeptname" placeholder="请输入调动后部门" />
        </el-form-item>
        <el-form-item label="调动后职位ID" prop="newdeptid">
          <el-input v-model="form.newdeptid" placeholder="请输入${comment}" />
        </el-form-item>
        <el-form-item label="调动后职位" prop="newranking">
          <el-input v-model="form.newranking" placeholder="请输入调动后职位" />
        </el-form-item>
        <el-form-item label="是否已完成" prop="isover">
...
<script>
import { listHrtransfer, getHrtransfer, delHrtransfer, addHrtransfer, updateHrtransfer } from "@/api/xinhu-hr/hrtransfer";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        tranuid: [
          { required: true, message: "要调动人ID不能为空", trigger: "blur" }
        ],
        tranname: [
          { required: true, message: "要调动人不能为空", trigger: "blur" }
        ],
        trantype: [
          { required: true, message: "调动类型不能为空", trigger: "change" }
        ],
        olddeptname: [
          { required: true, message: "原来部门不能为空", trigger: "blur" }
        ],
        oldranking: [
          { required: true, message: "原来职位不能为空", trigger: "blur" }
        ],
        effectivedt: [
        ...
        newdeptid: [
          { required: true, message: "调动后职位ID不能为空", trigger: "blur" }
        ],
        newranking: [
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除职位调动编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('xinhu-hr/hrtransfer/export', {
        ...this.queryParams
