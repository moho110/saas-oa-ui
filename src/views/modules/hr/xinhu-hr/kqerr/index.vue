  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['xinhu-hr:kqerr:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['xinhu-hr:kqerr:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['xinhu-hr:kqerr:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="kqerrList" @selection-change="handleSelectionChange">
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="180px">
        ...
        <el-form-item label="申请人" prop="uname">
          <el-input v-model="form.uname" placeholder="请输入申请人" />
        </el-form-item>
        ...
        <el-form-item label="异常类型" prop="errtype">
          <el-radio v-model="form.errtype" label="1">特殊类型</el-radio>
          <el-radio v-model="form.errtype" label="0">一般类型</el-radio>
        </el-form-item>
        <el-form-item label="异常日期" prop="dt">
          <el-date-picker clearable size="small"
            v-model="form.dt"
            type="date"
            value-format="yyyy-MM-dd"
        ...
        <el-form-item label="应打卡时间" prop="ytime">
          <el-input v-model="form.ytime" placeholder="请输入应打卡时间" />
        </el-form-item>
...
<script>
import { listKqerr, getKqerr, delKqerr, addKqerr, updateKqerr } from "@/api/xinhu-hr/kqerr";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        dt: [
          { required: true, message: "异常日期不能为空", trigger: "blur" }
        ],
        ytime: [
          { required: true, message: "应打卡时间不能为空", trigger: "blur" }
        ],
        comid: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listKqerr(this.queryParams).then(response => {
        this.kqerrList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateKqerr(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addKqerr(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除打卡异常申请编号为"' + ids + '"的数据项？').then(function() {
        return delKqerr(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('xinhu-hr/kqerr/export', {
        ...this.queryParams
      }, `kqerr_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
