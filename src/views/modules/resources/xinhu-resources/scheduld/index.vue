<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="待办人" prop="distren">
        <el-input
          v-model="queryParams.distren"
          placeholder="请输入待办人"
          clearable
          size="small"
          @keyup.enter.native="handleQuery"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['xinhu-resources:scheduld:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['xinhu-resources:scheduld:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['xinhu-resources:scheduld:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['xinhu-resources:scheduld:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="scheduldList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="日程时间" align="center" prop="startdt" width="180">
        <template slot-scope="scope">
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="180px">
        ...
        <el-form-item label="日程的id" prop="schid">
          <el-input v-model="form.schid" placeholder="请输入日程的id" />
        </el-form-item>
        <el-form-item label="提醒给ID" prop="receid">
          <el-input v-model="form.receid" type="textarea" placeholder="请输入提醒给ID" />
        </el-form-item>
        ...
        <el-form-item label="日程时间" prop="startdt">
          <el-date-picker clearable size="small"
        ...
        <el-form-item label="分配给人" prop="distid">
          <el-input v-model="form.distid" placeholder="请输入分配给人" />
        </el-form-item>
        <el-form-item label="待办人" prop="distren">
          <el-input v-model="form.distren" placeholder="请输入待办人" />
        </el-form-item>
      </el-form>
...
<script>
import { listScheduld, getScheduld, delScheduld, addScheduld, updateScheduld } from "@/api/xinhu-resources/scheduld";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        schid: [
          { required: true, message: "日程的id不能为空", trigger: "blur" }
        ],
        receid: [
          { required: true, message: "提醒给ID不能为空", trigger: "blur" }
        ],
        recename: [
        ...
        distid: [
          { required: true, message: "分配给人不能为空", trigger: "blur" }
        ],
        distren: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listScheduld(this.queryParams).then(response => {
        this.scheduldList = response.rows;
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateScheduld(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addScheduld(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除日程待办编号为"' + ids + '"的数据项？').then(function() {
        return delScheduld(ids);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('xinhu-resources/scheduld/export', {
        ...this.queryParams
      }, `scheduld_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
