    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['xinhu-resources:schedule:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['xinhu-resources:schedule:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['xinhu-resources:schedule:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['xinhu-resources:schedule:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="180px">
        ...
        <el-form-item label="频率" prop="rate">
          <el-input v-model="form.rate" placeholder="请输入频率" />
        </el-form-item>
        <el-form-item label="频率值" prop="rateval">
          <el-input v-model="form.rateval" placeholder="请输入频率值" />
        </el-form-item>
        <el-form-item label="是否提醒" prop="txsj">
          <el-input v-model="form.txsj" placeholder="请输入是否提醒" />
        </el-form-item>
        ...
        <el-form-item label="提醒给ID" prop="receid">
          <el-input v-model="form.receid" placeholder="请输入提醒给ID" />
        </el-form-item>
        <el-form-item label="提醒给" prop="recename">
          <el-input v-model="form.recename" placeholder="请输入提醒给" />
        </el-form-item>
        <el-form-item label="对应单位id" prop="comid">
          <el-input v-model="form.comid" placeholder="请输入对应单位id" />
        </el-form-item>
        <el-form-item label="是否创建日程待办" prop="isdai">
          <el-radio v-model="form.isdai" label="1">是</el-radio>
          <el-radio v-model="form.isdai" label="0">否</el-radio>
        </el-form-item>
      </el-form>
...
<script>
import { listSchedule, getSchedule, delSchedule, addSchedule, updateSchedule } from "@/api/xinhu-resources/schedule";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        rateval: [
          { required: true, message: "频率值不能为空", trigger: "blur" },
          { min: 3, max: 50, message: '长度在 3 到 50 个字符', trigger: 'blur' },
          { type: 'number', message: '频率值必须为数字值'}
        ],
        txsj: [
        ...
        isdai: [
          { required: true, message: "是否创建日程待办不能为空", trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除日程编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('xinhu-resources/schedule/export', {
        ...this.queryParams
